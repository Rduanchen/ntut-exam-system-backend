...
export class ScoreBoardService {
    ...
    async updateStudentScore(input: UpdateScoreInput) {
        ...
        let result = await this.mapResultToScoreBoardFormat(input.score, studentOriginalData.puzzle_results);
        try {
    ...
    async mapResultToScoreBoardFormat(data: Record<string, RunTestsResult>, baseScoreboard: any) {
        // 複製基底，保留未提供題目的原狀
        ...
        for (const [puzzleId, puzzleData] of Object.entries(data)) {
            ...
            const groups = Array.isArray(puzzleData.groupResults) ? puzzleData.groupResults : [];
            for (const group of groups) {
                if (!group || typeof group !== "object") continue;

                const groupId = group.id; // 如 1, 2
                const tcs = Array.isArray(group.testCasesResults) ? group.testCasesResults : [];
                for (const tc of tcs) {
